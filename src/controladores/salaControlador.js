...
controller.autoBorrado = (req, res) => {
        var hoy = new Date();
    ...
    req.getConnection((err, conn) => {
        ...
        conn.query("DELETE * FROM reservacion WHERE  id_sala in (select id_sala from reservacion where hora_final < ?)", [fechaYHora], (err, rows) => {
            console.log(err);
            res.redirect('/');
        });
    })
